...
2. If there is one, start it: `px tracker tasks start <id>`
...
4. Mark it as done: `px tracker tasks done <id>`
...
```bash
px tracker tasks done <your-task-id>
```
...
1. After completing a task, run: `px tracker tasks me` to see tasks assigned to you
2. If there is a task, run `px tracker tasks start <id>`, do the work, then `px tracker tasks done <id>`
...
IMPORTANT: the command `px tracker tasks list` does NOT exist. Use `px tracker tasks` instead.
